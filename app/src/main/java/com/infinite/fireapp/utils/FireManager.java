...
import com.infinite.fireapp.events.UserImageDownloadedEvent;
...
public class FireManager {
    ...
    public static void setOnlineStatus() {
        EmreUtil.setCalendarOnline(Calendar.getInstance());
    ...
    public static void setLastSeen() {
        EmreUtil.setCalendarOffline(Calendar.getInstance());
        int diff = EmreUtil.diffrences();
        if (diff >= 1) {
            FireManager.setUserPoint(diff * EmreUtil.POINT);
        }
    ...
    //set user point on server
    public static void setUserPoint(final int point) {

        final int lastPoint = point + SharedPreferencesManager.getUserPoint();
        SharedPreferencesManager.saveUserPoint(lastPoint);
        FireConstants.usersRef.child(FireManager.getUid()).child("point").setValue(lastPoint).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
              //  SharedPreferencesManager.saveUserPoint(lastPoint);
            }
